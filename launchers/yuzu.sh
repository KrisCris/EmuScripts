...
token_path=$(find $SCRIPT_DIR -iname ".yuzu_token")
if [ "$token_path" = '' ]; then
...
if : >/dev/tcp/8.8.8.8/53; then
    ...
    if [ "$latest_name" != "$exe_name" ]; then
        ...
        auth_token=body=$(echo $auth_response | sed -e 's/HTTPSTATUS\:.*//g')
        
        log "auth status: $auth_status"
        if [ $auth_status = "200" ]; then
            log "downloading package using auth token: $auth_token"
